...
1. 非线程安全
    这种情况可以通过使用并发版本的 Map 来缓解，比如 ConcurrentMap。
    另外，也可以通过 Lock 来处理 Map 的访问控制。各自权衡，不做评论
2. 同一个序列值可能被重复计算
    假如两个线程需要获取一个相同的序列值，但是这个序列值并不在缓存里面，
    此时，这两个线程都会各自去计算这个序列值，从而导致这个序列值被计算了两次。
    如下图所示：
        ![]()
    这些问题不仅违背了缓存的初衷，同时也浪费了计算资源，降低了系统的吞吐量。如下图：
        ![]()
    这个问题看起来无关紧要，但是却对系统资源（CPU，内存）有严重的影响。
    缓存的目的是通过从缓存中取值从而减少计算需求。

3. 这种设计难以移植到其他的算法设计中
    这个缓存设计被硬编码为针对单一算法的实现，在此例中，是斐波那契生成器。
    getNumber() 同时负责缓存数据和计算序列值，无法将这两种责任剥离开来。
    如果说我们想将缓存应用到别的场景，比如查看一个大数是不是质数，
    我们需要同时编写缓存逻辑和质数计算代码，结果导致我们不得不修改每一处用到货想用到缓存的地方。
    
一个改进的设计版本是使用 Callable 和 Future 来实现。上问已经提到，可以使用 ConcurrentMap
来缓存值。这三个类可以解决刚刚讨论的缺陷。
...
## 线程安全的通用缓存方案
为了将上面的简单缓存实现改造为线程安全，通用的实现，需要进行三处改动。
缓存必须是线程安全并足够智能的，当第一个线程还没有完成计算时，要避免第二个线程进行相同的计算。
当第一个线程完成计算之后，第二个线程直接使用第一个线程计算得来的结果。
我们还需要将与斐波那契计算相关的代码全部挪出缓存。同时，为了通用，我们还需要使用泛型来适应不同的场景。
...
```
这个实现尚不足 55 行，但是满足上述所有的需求。我们逐一分析：

1. 泛型参数 K 表示键类型，V 表示值类型，在斐波那契的例子中，K 和 V 都是 Long。
2. 缓存值都保存在 ConcurrentMap 中而不是单纯的 Map。更重要的是， cache 中的保存值不是直接的 V 类型值，
而是 Future，这种方式是避免重复计算的关键。
Future 表示将计算并获得一个值，对于每一个 key，都只会有一个对应的 Future，
当线程1将 Future 添加到缓存之后，线程2会获取同一个 Future，然后等待 Future 的结果返回。
这就解决了重复计算的问题，不过我们需要保证只有一个 Future 被添加到 cache 中。
3. 不论有多少个线程使用到了缓存，我们都需要保证一个 key 只有添加一个对应的 Future，下面的代码实现了这个目的：
...
4. 从 Future 获取最终需要用到的值，Future#get()
...
并将异常抛出给调用者来处理。
5. setValueIfAbsent() 方法使得用户可以添加那些还未产生的缓存值，
...
如上所述，改动很小。所有的缓存代码都封装在缓存算法中，只需要简单交互即可，
并且不需要考虑线程安全问题，从而专注在业务逻辑上。输出结果如下：
...
缓存算法并不依赖任何具体类，也就是说可以应用到任何场景中。这里我们有一个虚拟的耗时任务需要缓存：
